...
BASE_DIR = "models"
PROJECT_DIR = "GOE_5"
MODEL_NAME = "MLP - DistMult - Negative 1"
...
class ComplExModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types, dropout_rates=None):
        ...
        # === Real & Imaginary Embeddings ===
        self.entity_real_embeddings = nn.Embedding(num_entities, embedding_dim)
        self.entity_imag_embeddings = nn.Embedding(num_entities, embedding_dim)
        self.relation_real_embeddings = nn.Embedding(num_relations, embedding_dim)
        self.relation_imag_embeddings = nn.Embedding(num_relations, embedding_dim)
        ...
        # === MLP Classifier ===
        input_dim = 1  # ComplEx score is scalar
        ...
        # === Initialization ===
        for param in [
            self.entity_real_embeddings, self.entity_imag_embeddings,
            self.relation_real_embeddings, self.relation_imag_embeddings
        ]:
    ...
    def forward(self, x):
        ...
        # Apply GAT to real and imag parts
        base_real = self.entity_real_embeddings.weight
        base_imag = self.entity_imag_embeddings.weight

        refined_real = self.gat_layer(base_real, edge_index=self.edge_index,
                                      entity_types=self.entity_type_tensor.to(base_real.device))
        refined_imag = self.gat_layer(base_imag, edge_index=self.edge_index,
                                      entity_types=self.entity_type_tensor.to(base_imag.device))

        h_real = refined_real[x[:, 0]]
        h_imag = refined_imag[x[:, 0]]
        r_real = self.relation_real_embeddings(x[:, 1])
        r_imag = self.relation_imag_embeddings(x[:, 1])
        t_real = refined_real[x[:, 2]]
        t_imag = refined_imag[x[:, 2]]
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for j in range(0, len(X_val), batch_size):
                ...
                batch_input_val = torch.stack([h_val, r_val, t_val], dim=1)
                val_preds = model(batch_input_val)

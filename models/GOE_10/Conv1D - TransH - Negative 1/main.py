...
class TransHConvModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, projection_dim, num_types,
                 dropout_rate=0.5, type_embedding_dim=32, heads=2):
        super().__init__()

        self.embedding_dim = embedding_dim
        self.heads = heads
        self.post_gnn_dim = embedding_dim * heads  # 512 * 2 = 1024

        self.entity_embeddings = nn.Embedding(num_entities, embedding_dim)
        self.relation_embeddings = nn.Embedding(num_relations, embedding_dim * heads)
        self.relation_projections = nn.Embedding(num_relations, projection_dim * heads)
        ...
        self.gcn_layer = OntologyAwareGCNLayer(
            in_channels=embedding_dim,
            out_channels=self.post_gnn_dim,  # match GAT output
            num_types=num_types,
            type_embedding_dim=type_embedding_dim,
            dropout=0.3
        )
        ...
        # Conv1D + Dense
        self.conv1d_1 = nn.Conv1d(1, 128, kernel_size=3, padding=1)
        ...
        self.flatten_dim = 256 * 4 * self.post_gnn_dim  # 256 * 4096 = 1,048,576
    ...
    def forward(self, inputs):
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("edge_index and entity_type_tensor must be set.")
        ...
        refined_entity_emb = 0.5 * (gat_out + gcn_out)
        ...
        # Conv1D on [s_embed, p_embed, o_proj, predicted_o]
        concat = torch.cat([s_embed, p_embed, o_proj, predicted_o], dim=1)  # (B, 4096)
        x = concat.unsqueeze(1)  # (B, 1, 4096)

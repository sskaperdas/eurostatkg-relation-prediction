...
BASE_DIR = "models"
PROJECT_DIR = "GOE_10"
MODEL_NAME = "Conv1D - HoIE - Negative 1"
...
class ComplexModelWithHoIE(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, hoie_embedding_dim, num_types, dropout_rate=0.5):
        ...
        self.heads = 4
        self.post_gnn_dim = embedding_dim * self.heads  # 512 * 4 = 2048
    ...
    def forward(self, inputs):
        ...
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("GAT and GCN require edge_index and entity_type_tensor to be set.")

        # === Refine entity embeddings with GAT and GCN ===
        raw_ent_emb = self.entity_embeddings.weight  # [N, 512]
        gat_out = self.gat_layer(raw_ent_emb, self.edge_index, self.entity_type_tensor.to(raw_ent_emb.device))  # [N, 2048]
        gcn_out = self.gcn_layer(raw_ent_emb, self.edge_index, self.entity_type_tensor.to(raw_ent_emb.device))  # [N, 2048]
        refined_ent_emb = 0.5 * (gat_out + gcn_out)  # [N, 2048]

        s_embed = refined_ent_emb[s_idx]  # [B, 2048]
        p_embed = self.relation_embeddings(p_idx)  # [B, 512]
        o_embed = refined_ent_emb[o_idx]  # [B, 2048]

        # HoIE for subject
        hoie_s = self.hoie_embeddings(s_idx)
        s_real, s_imag = torch.chunk(hoie_s, 2, dim=-1)
        s_hoie = self.hoie_dense2(F.relu(self.hoie_dense1(self.combine_hoie(s_real, s_imag))))  # [B, H]

        # HoIE for object
        hoie_o = self.hoie_embeddings(o_idx)
        o_real, o_imag = torch.chunk(hoie_o, 2, dim=-1)
        o_hoie = self.hoie_dense2(F.relu(self.hoie_dense1(self.combine_hoie(o_real, o_imag))))  # [B, H]

        # === Concatenate all ===
        x = torch.cat([s_embed, p_embed, o_embed, s_hoie, o_hoie], dim=1)  # [B, 2048 + 512 + 2048 + H + H]
        ...
        return torch.sigmoid(self.dense2(x))  # [B, 1]

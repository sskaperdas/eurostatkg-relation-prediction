...
class DistMultModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        refined_entity_emb = 0.5 * (gat_out + gcn_out)  # [num_entities, 2048]

        s_embed = refined_entity_emb[s_idx]  # [B, 2048]
        p_embed = self.relation_embeddings(p_idx)  # [B, 2048]
        o_embed = refined_entity_emb[o_idx]  # [B, 2048]

        # DistMult scoring with 2048-dim
        score = torch.sum(s_embed * p_embed * o_embed, dim=-1)  # [B]
        x = score.unsqueeze(1).unsqueeze(2)  # [B, 1, 1]

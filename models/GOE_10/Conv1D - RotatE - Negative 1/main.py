...
class ComplexModelWithRotatE(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, rot_embedding_dim, num_types, dropout_rate=0.5, l2_reg=0.01):
        ...
        self.heads = 2
        self.post_gnn_dim = embedding_dim * self.heads  # 512 * 2 = 1024

        # Base entity embeddings (for GAT/GCN)
        self.entity_embeddings = nn.Embedding(num_entities, embedding_dim * 2)

        # RotatE embeddings (remain unchanged)
        ...
        self.relation_embeddings = nn.Embedding(num_relations, self.post_gnn_dim)
        ...
        self.gcn_layer = OntologyAwareGCNLayer(
            in_channels=embedding_dim * 2,
            out_channels=self.post_gnn_dim,
            num_types=num_types,
            type_embedding_dim=32,
            dropout=0.3
        )
    ...
    def forward(self, inputs):
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("GAT/GCN require edge_index and entity_type_tensor to be set.")

        s_idx, p_idx, o_idx = inputs[:, 0], inputs[:, 1], inputs[:, 2]

        # === Refine entity embeddings using both GAT and GCN ===
        base_entity_emb = self.entity_embeddings.weight  # shape: [num_entities, 1024]
        ...
        refined_entity_emb = 0.5 * (gat_out + gcn_out)  # shape: [num_entities, 1024]

        s_embed = refined_entity_emb[s_idx]
        p_embed = self.relation_embeddings(p_idx)  # shape: [batch_size, 1024]
        ...
        scores = torch.stack([re_score, im_score], dim=1)  # shape: [batch, 2]
        x = scores.unsqueeze(1)  # [batch, 1, 2]

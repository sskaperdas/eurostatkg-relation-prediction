...
class TransEConvModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types, dropout_rate=0.5, heads=2):
        ...
        self.gcn_layer = OntologyAwareGCNLayer(
            in_channels=embedding_dim,
            out_channels=embedding_dim * heads,  # match GAT output dim
            num_types=num_types,
            type_embedding_dim=32,
            dropout=dropout_rate
        )
        ...
        self.post_gat_dim = embedding_dim * heads  # Used by both GAT and GCN
    ...
    def forward(self, inputs):
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("Both edge_index and entity_type_tensor must be set before calling forward()")

        base_entity_emb = self.entity_embeddings.weight

        gat_out = self.gat_layer(
            base_entity_emb,
            edge_index=self.edge_index,
            entity_types=self.entity_type_tensor.to(base_entity_emb.device)
        )

        gcn_out = self.gcn_layer(
            base_entity_emb,
            edge_index=self.edge_index,
            entity_types=self.entity_type_tensor.to(base_entity_emb.device)
        )

        # === Fusion: mean of GAT and GCN outputs ===

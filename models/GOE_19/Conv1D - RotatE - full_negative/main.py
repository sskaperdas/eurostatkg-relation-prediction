...
BASE_DIR = "models"
PROJECT_DIR = "GOE_19"
MODEL_NAME = "Conv1D - RotatE - full_negative"
...
class ComplexModelWithRotatE(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, rot_embedding_dim, num_types, dropout_rate=0.5, l2_reg=0.01):
        super(ComplexModelWithRotatE, self).__init__()
        ...
        # RotatE embeddings
        self.rot_entity_embeddings = nn.Embedding(num_entities, rot_embedding_dim * 2)
        self.rot_relation_embeddings = nn.Embedding(num_relations, rot_embedding_dim)
        ...
        self.gat_layer = OntologyAwareGATLayer(
            ...
            heads=2,  # can increase to 2+ if desired
            dropout=0.3
        )
    ...
    def forward(self, inputs):
        ...
        # === Refine entity embeddings with GAT ===
        base_entity_emb = self.entity_embeddings.weight  # shape: [num_entities, 2 * emb_dim]
        ...
        s_real, s_imag = torch.chunk(rot_s_embed, 2, dim=-1)
        o_real, o_imag = torch.chunk(rot_o_embed, 2, dim=-1)

        re_score = torch.sum(rot_p_embed * (s_real * o_real + s_imag * o_imag), dim=-1)
        im_score = torch.sum(rot_p_embed * (s_imag * o_real - s_real * o_imag), dim=-1)

        scores = torch.stack([re_score, im_score], dim=1)  # shape: (batch_size, 2)
        x = scores.unsqueeze(1)  # shape: (batch_size, 1, 2)
        ...
        if self.dense1 is None:
            ...
            self.dense1 = nn.Linear(self.flatten_size, 512).to(x.device)
    ...
    def score_triple(self, h, r, t):
        ...
        full_entity_emb = self.entity_embeddings.weight  # [num_entities, 2 * embedding_dim]
        base_real, base_imag = torch.chunk(full_entity_emb, 2, dim=1)
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    """
    Train model with HybridLossWithSemantics (BCE + margin ranking + semantic penalty).
    Includes early stopping, validation batching, loss plotting, and file saving.
    """
    ...
    for epoch in range(epochs):
        ...
        for i in tqdm_bar:
            ...
            # Hybrid loss only if we have both pos and neg samples
            pos_mask = (batch_y == 1)
            neg_mask = (batch_y == 0)
            if pos_mask.sum() > 0 and neg_mask.sum() > 0:
                pos_triples = batch_X[pos_mask][:min(pos_mask.sum(), neg_mask.sum())]
                neg_triples = batch_X[neg_mask][:min(pos_mask.sum(), neg_mask.sum())]
                loss = loss_fn(
                    pred_probs=preds,
        ...
        with torch.no_grad():
            for j in range(0, len(X_val), batch_size):
                ...
                batch_loss = bce_loss_only(val_preds, batch_y_val.view(-1).float())
...
classifier = ComplexModelWithRotatE    (
    num_entities=num_entities,
    num_relations=num_relations,
    embedding_dim=EMBEDDING_DIM,
    rot_embedding_dim=EMBEDDING_DIM,  # you must define this too

...
BASE_DIR = "models"
PROJECT_DIR = "GOE_19"
MODEL_NAME = "Conv1D - TransH - full_negative"
...
class TransHConvModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, projection_dim, num_types, dropout_rate=0.5, type_embedding_dim=32, heads=1):
        super(TransHConvModel, self).__init__()

        self.embedding_dim = embedding_dim
        self.entity_embeddings = nn.Embedding(num_entities, embedding_dim)
        self.relation_embeddings = nn.Embedding(num_relations, embedding_dim)
        self.relation_projections = nn.Embedding(num_relations, projection_dim)
        ...
        self.flatten_dim = 256 * 4 * embedding_dim  # Adjusted after 1D conv on (1, 4*embedding_dim)
    ...
    def forward(self, inputs):
        ...
        # === Project to relation-specific hyperplane (TransH logic) ===
        s_proj = s_embed - torch.sum(s_embed * p_proj, dim=-1, keepdim=True) * p_proj
        o_proj = s_proj + p_embed
        predicted_o = s_embed + p_embed

        # === Conv1D on concatenated features ===
        concat = torch.cat([s_embed, p_embed, o_proj, predicted_o], dim=1)  # Shape: (batch_size, 4 * emb_dim)
        x = concat.unsqueeze(1)  # Shape: (batch_size, 1, 4 * emb_dim)
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    Includes early stopping, learning rate scheduling, logging, plotting, and saving.
    """

...
BASE_DIR = "models"
PROJECT_DIR = "GOE_19"
MODEL_NAME = "Conv1D - TransE - full_negative"
...
class TransEConvModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types, dropout_rate=0.5, heads=2):
        super(TransEConvModel, self).__init__()

        self.entity_embeddings = nn.Embedding(num_entities, embedding_dim)
        self.relation_embeddings = nn.Embedding(num_relations, embedding_dim * heads)
        ...
        self.post_gat_dim = embedding_dim * heads  # FIX: embeddings are bigger after GAT!
    ...
    def forward(self, inputs):
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("GAT requires edge_index and entity_type_tensor to be set before calling forward()")
        ...
        s_embed = refined_entity_emb[inputs[:, 0]]
        p_embed = self.relation_embeddings(inputs[:, 1])
        o_embed = refined_entity_emb[inputs[:, 2]]

        predicted_o_embed = s_embed + p_embed
        concatenated = torch.cat([s_embed, p_embed, o_embed, predicted_o_embed], dim=1)  # (batch_size, 4 * post_gat_dim)

        x = concatenated.unsqueeze(1)  # (batch_size, 1, 4 * post_gat_dim)
        ...
        if self.dense2 is None:
            self.dense2 = nn.Linear(512, 1).to(x.device)
    ...
    def score_triple(self, h, r, t):
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("GCN inputs (edge_index, entity_type_tensor) must be set before scoring.")

        base_entity_emb = self.entity_embeddings.weight
        gat_out = self.gat_layer(base_entity_emb, self.edge_index, self.entity_type_tensor.to(base_entity_emb.device))
        gcn_out = self.gcn_layer(base_entity_emb, self.edge_index, self.entity_type_tensor.to(base_entity_emb.device))
        refined_emb = (gat_out + gcn_out) / 2

        h_embed = refined_emb[h]
        t_embed = refined_emb[t]
        r_embed = self.relation_proj(self.relation_embeddings(r))
        ...
        predicted_t = h_embed + r_embed
        x = torch.cat([h_embed, r_embed, t_embed, predicted_t], dim=1)

        for i, (fc, norm, dropout, proj) in enumerate(zip(
            self.fc_layers, self.norm_layers, self.dropout_layers, self.residual_projections
        )):
            residual = proj(x)
            x = fc(x)
            if i % 3 == 0:
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    Includes early stopping, learning rate scheduling, loss plotting, and best model checkpointing.
    """
...
classifier = TransEConvModel(
    ...
    num_relations=num_relations,
    embedding_dim=256,
    num_types=num_types,  # pass number of types
    dropout_rate=0.5,
    heads=2  # or 1 if you want
).to(DEVICE)
...
def build_entity_type_tensor(entity_type_map_idx, num_entities):
    ...
    # Replace incorrect reference to classifier
    print("entity_type_tensor being built:", type_tensor.shape)

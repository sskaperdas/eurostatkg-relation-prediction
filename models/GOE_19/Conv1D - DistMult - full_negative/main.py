...
BASE_DIR = "models"
PROJECT_DIR = "GOE_19"
MODEL_NAME = "Conv1D - DistMult - full_negative"
...
class DistMultModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types, dropout_rate=0.5, l2_reg=0.01):
        ...
        self.gat_layer = OntologyAwareGATLayer(
            ...
            out_channels=embedding_dim,  # <- Ensure final dim = embedding_dim
            num_types=num_types,
            type_embedding_dim=32,
            heads=4,  # You can still use 4
            dropout=0.3
        )
    ...
    def score_triple(self, h, r, t):
        ...
        # DistMult score
        score = torch.sum(h_emb * r_emb * t_emb, dim=1, keepdim=True)  # (batch_size, 1, 1)
        x = score.unsqueeze(2)  # (batch_size, 1, 1)
        ...
        return self.dense2(x).squeeze(-1)  # Raw score

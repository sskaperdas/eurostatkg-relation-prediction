...
BASE_DIR = "models"
PROJECT_DIR = "GOE_19"
MODEL_NAME = "Conv1D - HoIE - full_negative"
...
class ComplexModelWithHoIE(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, hoie_embedding_dim, num_types, dropout_rate=0.5):
        ...
        self.hoie_embeddings = nn.Embedding(num_entities, hoie_embedding_dim * 2)  # real + imag

        self.hoie_dense1 = nn.Linear(hoie_embedding_dim, hoie_embedding_dim)
    ...
    def combine_hoie(self, real, imag):
        return real * imag  # Simplified multiplicative fusion
    ...
    def forward(self, inputs):
        ...
        raw_ent_emb = self.entity_embeddings.weight
        refined_ent_emb = self.gat_layer(
            raw_ent_emb,
            edge_index=self.edge_index,
            entity_types=self.entity_type_tensor.to(raw_ent_emb.device)
        )

        s_embed = refined_ent_emb[s_idx]
        p_embed = self.relation_embeddings(p_idx)
        o_embed = refined_ent_emb[o_idx]

        # HoIE for subject
        hoie_s = self.hoie_embeddings(s_idx)
        s_real, s_imag = torch.chunk(hoie_s, 2, dim=-1)
        s_hoie = self.hoie_dense2(F.relu(self.hoie_dense1(self.combine_hoie(s_real, s_imag))))

        # HoIE for object
        hoie_o = self.hoie_embeddings(o_idx)
        o_real, o_imag = torch.chunk(hoie_o, 2, dim=-1)
        o_hoie = self.hoie_dense2(F.relu(self.hoie_dense1(self.combine_hoie(o_real, o_imag))))

        # Concatenate all
        x = torch.cat([s_embed, p_embed, o_embed, s_hoie, o_hoie], dim=1)  # (B, D_total)
        x = x.unsqueeze(1)  # (B, 1, D_total)
    ...
    def score_triple(self, h, r, t):
        ...
        using HoIE-style circular correlation and MLP, consistent with forward().
        """
        if self.edge_index is None or self.entity_type_tensor is None:
        ...
        gat_raw = self.gat_layer(base_embeddings, self.edge_index, entity_types)
        gat_refined = self.gat_proj(gat_raw)

        gcn_refined = self.gcn_layer(base_embeddings, self.edge_index, entity_types)
        refined_embeddings = (gat_refined + gcn_refined) / 2

        s = refined_embeddings[h]
        o = refined_embeddings[t]
        ...
        return self.fc_out(x).squeeze(-1)  # raw score (pre-sigmoid)
...
classifier = ComplexModelWithHoIE(
    num_entities=num_entities,
    num_relations=num_relations,

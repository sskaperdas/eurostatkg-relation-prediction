...
BASE_DIR = "models"
PROJECT_DIR = "GOE_11"
MODEL_NAME = "MLP - TransE - full_negative - wn18"
...
class TransEModel(nn.Module):
    ...
    def score_triple(self, h, r, t):
        """
        Computes TransE-style plausibility score for triples: -||h + r - t||_2
        """
        h_embed = self.entity_embeddings(h)
        r_embed = self.relation_embeddings(r)
        t_embed = self.entity_embeddings(t)
        # Include structural features just like in forward()
        h_embed += self.in_degree(h) + self.inverse_relation_frequency(r)
        t_embed += self.out_degree(t) + self.inverse_relation_frequency(r)
        # Optional: Normalize for stability
        h_embed = self.batch_norm(h_embed)
        t_embed = self.batch_norm(t_embed)
        return -torch.norm(h_embed + r_embed - t_embed, p=2, dim=1)  # Shape: [batch_size]
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    # Reduce LR by a factor of 0.5 if validation loss doesn't improve for 2 epochs
    ...
    for epoch in range(epochs):
        ...
        for i in progress_bar:
            ...
            if pos_mask.sum() > 0 and neg_mask.sum() > 0:
                ...
                # Ensure equal number of pos and neg triples for margin ranking
        ...
        # === Validation (BCE only for fairness) ===
        model.eval()
        val_loss = 0.0
        val_batches = 0
        with torch.no_grad():
...
def evaluate_model(model, X_test, y_test, model_name="TransEModel", batch_size=512):
    ...
    return metrics_results, predictions.numpy(), y_test_cpu.numpy()
# Call the evaluation function and save results
model_metrics, y_scores, y_true = evaluate_model(trained_classifier, X_test, y_test, model_name=MODEL_NAME)

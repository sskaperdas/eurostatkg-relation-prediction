...
class DistMultModel(nn.Module):
    ...
    def forward(self, h, r, t):
        # Input: individual indices (batch_size,)
        ...
        output = torch.sigmoid(self.out(x))  # (batch_size, 1)
        return output

    def score_triple(self, h, r, t):
        """
        Computes DistMult plausibility score for triples: ⟨h, r, t⟩ = hᵗ * diag(r) * t
        """
        h_embed = self.entity_embeddings(h)
        r_embed = self.relation_embeddings(r)
        t_embed = self.entity_embeddings(t)

        return torch.sum(h_embed * r_embed * t_embed, dim=1)  # shape: [batch_size]
...
classifier = DistMultModel(num_entities, num_relations, EMBEDDING_DIM).to(DEVICE)

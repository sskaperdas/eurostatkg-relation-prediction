...
class TransHModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim):
        """
        Enhanced TransH + MLP Model with:
        ...
        self.hyperplane_embeddings = nn.Embedding(num_relations, embedding_dim * embedding_dim)
        ...
        for i in range(len(layer_sizes) - 1):
            ...
            dropout_rate = 0.5 if i < 2 else 0.3 if i < 4 else 0.2  # Adaptive Dropout Strategy
    ...
    def forward(self, h, r, t):
        ...
        # **Hyperplane Projection (TransH)**
        hyperplane_embed = self.hyperplane_embeddings(r).view(-1, h_embed.shape[1], h_embed.shape[1])
        h_projected = torch.bmm(hyperplane_embed, h_embed.unsqueeze(-1)).squeeze(-1)
        t_projected = torch.bmm(hyperplane_embed, t_embed.unsqueeze(-1)).squeeze(-1)

        # **Batch Normalization**
        h_projected = self.batch_norm(h_projected)
        t_projected = self.batch_norm(t_projected)

        # **Compute Predicted Tail Embedding**
        predicted_t_embed = h_projected + r_embed

        # **Concatenate Features**
        x = torch.cat([h_embed, r_embed, t_projected, predicted_t_embed], dim=1)
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    """
    Train model using HybridLossWithSemantics with optional ranking and semantic penalty.
    """
    ...
    for epoch in trange(epochs, desc="Training Epochs"):
        ...
        for i in tqdm(range(0, len(X_train), batch_size), leave=False, desc=f"Epoch {epoch + 1}/{epochs}"):
            ...
            # Ranking-aware loss if both classes present
            pos_mask = (batch_y == 1)
            neg_mask = (batch_y == 0)
            ...
            if lambda_rank > 0 and pos_mask.sum() > 0 and neg_mask.sum() > 0:
                ...
                # Fall back to BCE + semantic loss only
                loss = loss_fn(
                    pred_probs=preds,
                    true_labels=batch_y,
                    model=model,
...
classifier = TransHModel(num_entities, num_relations, EMBEDDING_DIM).to(DEVICE)

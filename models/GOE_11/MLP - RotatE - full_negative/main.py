...
class RotatE_MLP_Model(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, dropout_rates=None):
        """
        RotatE Model with:
        - Complex-valued RotatE embeddings
        ...
        self.relation_embeddings = nn.Embedding(num_relations, embedding_dim)  # Stores angles in radians

        # **Batch Normalization for input embeddings**
        self.batch_norm = nn.BatchNorm1d(embedding_dim * 4)  # Fix: Correct number of features
    ...
    def forward(self, h, r, t):
        """
        Forward Pass with RotatE embeddings and MLP classifier.
        - Complex-valued embeddings
        - Residual Connections
        - Batch Normalization
        - Adaptive Dropout
        - Swish/GELU Activations
        """
        ...
        # **Get Relation Rotation Angles**
        theta = self.relation_embeddings(r) * self.pi  # Convert to radians
        r_real = torch.cos(theta)
        r_imag = torch.sin(theta)

        # **Normalize Complex Embeddings**
        h_modulus = torch.sqrt(h_real ** 2 + h_imag ** 2 + 1e-9)
        h_real = h_real / h_modulus
        h_imag = h_imag / h_modulus

        t_modulus = torch.sqrt(t_real ** 2 + t_imag ** 2 + 1e-9)
        t_real = t_real / t_modulus
        t_imag = t_imag / t_modulus

        # **RotatE Transformation**
        rot_t_real = h_real * r_real - h_imag * r_imag
        rot_t_imag = h_real * r_imag + h_imag * r_real

        # **Compute Distance Between Rotated h and t**
        diff_real = rot_t_real - t_real
        diff_imag = rot_t_imag - t_imag

        # **L2 Norm as Scoring Function**
        score = torch.norm(diff_real, p=2, dim=-1) + torch.norm(diff_imag, p=2, dim=-1)

        # **Concatenate embeddings for MLP Input**
    ...
    def score_triple(self, h, r, t):
        """
        Computes RotatE-style plausibility score: -||h ◦ r - t|| where ◦ is complex rotation.
        """
        h_real = self.entity_embeddings_real(h)
        h_imag = self.entity_embeddings_imag(h)
        t_real = self.entity_embeddings_real(t)
        ...
        theta = self.relation_embeddings(r) * self.pi  # Convert angle embedding to radians
        r_real = torch.cos(theta)
        r_imag = torch.sin(theta)

        # Normalize complex components (as in forward)
        h_modulus = torch.sqrt(h_real ** 2 + h_imag ** 2 + 1e-9)
        h_real = h_real / h_modulus
        h_imag = h_imag / h_modulus
        ...
        # Apply rotation: h ◦ r
        rot_h_real = h_real * r_real - h_imag * r_imag
        rot_h_imag = h_real * r_imag + h_imag * r_real

        # Score = L2 distance between rotated h and t
        diff_real = rot_h_real - t_real
        diff_imag = rot_h_imag - t_imag
        return -torch.norm(diff_real, p=2, dim=1) - torch.norm(diff_imag, p=2, dim=1)
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    """
    Train using HybridLossWithSemantics:
    - BCE for classification
    - Semantic penalty from ontology (gamma_sem)
    - Margin ranking loss (lambda_rank)
    """
    ...
    for epoch in range(epochs):
        ...
        for i in tqdm_bar:
            ...
            if lambda_rank > 0 and pos_mask.sum() > 0 and neg_mask.sum() > 0:
        ...
        with torch.no_grad():
            for j in range(0, len(X_val), batch_size):
                ...
                batch_loss = bce_loss_only(val_preds, batch_y_val)
...
classifier = RotatE_MLP_Model(num_entities, num_relations, EMBEDDING_DIM).to(DEVICE)

...
BASE_DIR = "models"
PROJECT_DIR = "GOE_11"
MODEL_NAME = "MLP - TransE - full_negative - ranking"
...
def train_ranking_model(model, X_train, y_train, X_val, y_val,
                        ...
                        patience=PATIENCE, margin=1.0):
    """
    Train a RANKING model using MarginRankingLoss.
    This version includes early stopping, a progress bar, and loss plotting.
    """
    ...
    for epoch in range(epochs):
        ...
        # Use a DataLoader for easy batching and shuffling
        ...
        for batch_X, batch_y in progress_bar:
            pos_triples = batch_X[batch_y == 1].to(DEVICE)
            neg_triples = batch_X[batch_y == 0].to(DEVICE)
            ...
            # Get raw scores from the model (it should NOT have a sigmoid)
            pos_scores = model(pos_triples[:, 0], pos_triples[:, 1], pos_triples[:, 2]).squeeze()
            neg_scores = model(neg_triples[:, 0], neg_triples[:, 1], neg_triples[:, 2]).squeeze()

            # The target is a tensor of 1s, indicating we want pos_scores > neg_scores
        ...
        # === Validation Loop (also uses margin loss) ===
        model.eval()
        val_loss = 0.0
        val_batches = 0
        ...
        with torch.no_grad():
            for batch_X_val, batch_y_val in val_loader:
                pos_val = batch_X_val[batch_y_val == 1].to(DEVICE)
                neg_val = batch_X_val[batch_y_val == 0].to(DEVICE)
                ...
                pos_scores_val = model(pos_val[:, 0], pos_val[:, 1], pos_val[:, 2]).squeeze()
                neg_scores_val = model(neg_val[:, 0], neg_val[:, 1], neg_val[:, 2]).squeeze()
                target_val = torch.ones_like(pos_scores_val)

                batch_loss = loss_fn(pos_scores_val, neg_scores_val, target_val)
        ...
        if avg_val_loss < best_val_loss:
            ...
            with open(os.path.join(FULL_MODEL_DIR, "best_ranking_epoch.txt"), "w") as f:
                f.write(f"{epoch + 1}\n")
        else:
            patience_counter += 1
        ...
        if patience_counter >= patience:
            ...
            with open(os.path.join(FULL_MODEL_DIR, "last_ranking_epoch.txt"), "w") as f:
                f.write(f"{epoch + 1}\n")
            break
    ...
    plt.close()
    print(f" Ranking & Validation loss plot saved to {os.path.join(plots_dir, 'loss_plot_ranking.png')}")

    # **Load the best model**
    model.load_state_dict(torch.load(os.path.join(FULL_MODEL_DIR, "best_ranking_model.pth")))
    print(f" Best ranking model from epoch {epoch + 1 - patience_counter} loaded for evaluation.")
    return model
...
trained_classifier = train_ranking_model(
    ...
    patience=PATIENCE,
    margin=1.0  # You can tune this margin value
)
...
def calculate_hits_metrics(model, X_test, y_test, epsilons=None, batch_size=512):
    ...
    with torch.no_grad():
        # --- TQDM Added Here ---
        for i in tqdm(range(0, len(X_test), batch_size), desc="Calculating Hits@k Metrics"):
            batch_X = X_test[i:i + batch_size].to(DEVICE)
            batch_y = y_test[i:i + batch_size].to(DEVICE)
    ...
    # Compile results
    results = {
        f"hits@{k}": strict_hits[k] / total if total > 0 else 0.0 for k in [1, 5, 10]
    }

    for ε in epsilons:
        for k in [1, 5, 10]:
            results[f"soft_hits@{k}_eps={ε}"] = soft_hits[ε][k] / total if total > 0 else 0.0

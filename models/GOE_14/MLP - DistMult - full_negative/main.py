...
class DistMultModel(nn.Module):
    ...
    def forward(self, x):
        ...
        refined_entity_emb = self.gcn_layer(
            base_entity_emb,
            edge_index=self.edge_index,
            entity_types=self.entity_type_tensor.to(base_entity_emb.device)
        )

        # Extract entity embeddings after GCN
        h = refined_entity_emb[x[:, 0]]
        r = self.relation_embeddings(x[:, 1])
        t = refined_entity_emb[x[:, 2]]

        # DistMult scoring
        score = torch.sum(h * r * t, dim=1, keepdim=True)  # (batch_size, 1)
        ...
        return torch.sigmoid(self.out(x))  # (batch_size, 1)

    def score_triple(self, h, r, t):
        """
        Computes DistMult plausibility score: score = <h, r, t> = sum(h * r * t).
        Returns raw score (before sigmoid), shape: [batch_size]
        """
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("GCN edge_index and entity_type_tensor must be set before calling score_triple().")

        # === Refine entity embeddings via GCN ===
        base_entity_emb = self.entity_embeddings.weight
        refined_entity_emb = self.gcn_layer(
            base_entity_emb,
            edge_index=self.edge_index,
        ...
        # === DistMult scoring ===
        score = torch.sum(h_emb * r_emb * t_emb, dim=1)  # shape: [batch_size]
        return score  # Raw score (before sigmoid)
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    """
    Train model using HybridLossWithSemantics (BCE + margin ranking + semantic penalty).
    Compatible with models expecting batched [h, r, t] input like MLPHoIE.
    """
    ...
    for epoch in range(epochs):
        ...
        for i in tqdm_bar:
            ...
            optimizer.zero_grad()
            preds = model(batch_input).view(-1)
        ...
        with torch.no_grad():
            for j in range(0, len(X_val), batch_size):
                ...
                val_preds = model(batch_input_val).view(-1)
    ...
    print(f"Best model from epoch {epoch + 1 - patience_counter} loaded.")
    return model

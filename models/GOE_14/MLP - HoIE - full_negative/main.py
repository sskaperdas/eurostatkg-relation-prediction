...
class MLPHoIE(nn.Module):
    ...
    def forward(self, h, r, t):
        # === Ensure GCN Inputs are Set ===
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("GCN requires edge_index and entity_type_tensor to be set before forward().")

        # === Apply GCN to entity embeddings ===
        ...
        # === Use refined embeddings for head and tail ===
        s = refined_embeddings[h]
        o = refined_embeddings[t]

        # Relation embedding stays unchanged
    ...
    def score_triple(self, h, r, t):
        """
        Computes HoIE-style plausibility score for (h, r, t):
        score = MLP([circ(s), circ(p), circ(o)]) before sigmoid
        """
        if self.edge_index is None or self.entity_type_tensor is None:

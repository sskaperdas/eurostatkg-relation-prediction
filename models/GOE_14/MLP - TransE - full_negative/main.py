...
class TransEModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types,
                 gcn_output_dim=256, dropout_rates=None):
        """
        Enhanced TransE + Ontology-Aware GCN + MLP model with:
        - GCN-based entity embedding refinement using domain/range semantics
        - Residual connections
        - Adaptive dropout and advanced activations
        """
        ...
        # Normalization
        self.batch_norm = nn.BatchNorm1d(gcn_output_dim)

        # MLP Layer Sizes
        layer_sizes = [gcn_output_dim * 4, 2048, 1024, 512, 256, 128]
        ...
        # External inputs (must be set before training)
    ...
    def forward(self, h, r, t):
        ...
        # === Apply GCN to entire embedding space ===
        base_entity_embed = self.entity_embeddings.weight  # [num_entities, embedding_dim]
        gcn_output = self.gcn(base_entity_embed, self.edge_index, self.entity_type_tensor.to(base_entity_embed.device))

        # === Get embeddings for batch ===
        h_embed = gcn_output[h]
        r_embed = self.relation_embeddings(r)
        t_embed = gcn_output[t]
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    """
    Train model with BCE + Margin Ranking + Semantic Penalty.
    Early stopping and plotting included.
    """

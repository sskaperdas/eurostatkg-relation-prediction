...
class RotatE_MLP_Model(nn.Module):
    ...
    def score_triple(self, h, r, t):
        """
        Computes RotatE plausibility score for (h, r, t):
        score = -|| (h ∘ r) - t ||_2 in complex space.
        """
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("edge_index and entity_type_tensor must be set before calling score_triple()")

        # === Get GCN-refined entity embeddings (real + imaginary) ===
        base_real = self.entity_embeddings_real.weight
        base_imag = self.entity_embeddings_imag.weight

        refined_real = self.gcn_layer(base_real, self.edge_index, self.entity_type_tensor.to(base_real.device))
        refined_imag = self.gcn_layer(base_imag, self.edge_index, self.entity_type_tensor.to(base_imag.device))
        ...
        t_real = refined_real[t]
        t_imag = refined_imag[t]
        theta = self.relation_embeddings(r) * self.pi  # Relation = rotation angle

        r_real = torch.cos(theta)
        r_imag = torch.sin(theta)

        # === Normalize for unit modulus (optional but common in RotatE) ===
        h_mod = torch.sqrt(h_real ** 2 + h_imag ** 2 + 1e-9)
        t_mod = torch.sqrt(t_real ** 2 + t_imag ** 2 + 1e-9)
        h_real = h_real / h_mod
        h_imag = h_imag / h_mod
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    """
    Train model using BCE + Margin Ranking + Semantic Penalty loss with early stopping and visualization.
    """
    ...
    plt.close()
    print(f" Training & Validation loss plot saved.")

...
class ComplExModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, dropout_rate=0.5):
        ...
        # Dummy tensor to determine flattened size
        dummy_input = torch.zeros(1, 1, embedding_dim * 4)
        dummy_output = self.conv1d_2(self.dropout1(F.relu(self.batch_norm1(self.conv1d_1(dummy_input)))))
    ...
    def forward(self, inputs):
        ...
        s_real = self.entity_real_embeddings(s_idx)
        s_imag = self.entity_imag_embeddings(s_idx)
...
classifier = ComplExModel(num_entities, num_relations, EMBEDDING_DIM).to(DEVICE)

...
class DistMultModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, dropout_rate=0.5, l2_reg=0.01):
        ...
        # Flatten dimension: after Conv1D, output shape is (batch_size, 256, 1)
    ...
    def forward(self, inputs):
        ...
        s_embed = self.entity_embeddings(s_idx)  # (batch, dim)
        ...
        x = self.conv1d_1(x)                  # (batch, 128, 1)
        x = self.batch_norm1(x)
        x = F.relu(x)
        x = self.dropout1(x)

        x = self.conv1d_2(x)                  # (batch, 256, 1)

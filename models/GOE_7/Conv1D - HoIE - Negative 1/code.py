...
class ComplexModelWithHoIE(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, hoie_embedding_dim, dropout_rate=0.5):
        ...
        # Dense layers
        self.dense1 = None  # Will be lazily initialized on first forward pass
        self.batch_norm3 = nn.BatchNorm1d(512)
        self.dropout3 = nn.Dropout(dropout_rate)
        self.dense2 = nn.Linear(512, 1)

    def combine_hoie(self, real, imag):
        # This simulates circular correlation: h ⊛ t = IFFT(FFT(h) * conj(FFT(t)))
        # Since it's dense, we just do a basic multiplicative fusion for now
        return real * imag  # Simplified version
    ...
    def forward(self, inputs):
        ...
        s_real, s_imag = hoie_s[:, :self.hoie_embedding_dim], hoie_s[:, self.hoie_embedding_dim:]
        s_hoie = self.hoie_dense2(F.relu(self.hoie_dense1(self.combine_hoie(s_real, s_imag))))

        # HoIE for object
        hoie_o = self.hoie_embeddings(o_idx)
        o_real, o_imag = hoie_o[:, :self.hoie_embedding_dim], hoie_o[:, self.hoie_embedding_dim:]
...
classifier = ComplexModelWithHoIE(num_entities, num_relations, EMBEDDING_DIM, EMBEDDING_DIM).to(DEVICE)

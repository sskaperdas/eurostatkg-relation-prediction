...
BASE_DIR = "models"
PROJECT_DIR = "GOE_16"
MODEL_NAME = "MLP - TransH - full_negative - wn18"
FULL_MODEL_DIR = os.path.join(BASE_DIR, PROJECT_DIR, MODEL_NAME)
WN18RR_DIR = "models/GOE_16/MLP - TransH - full_negative - wn18"
...
def load_wn18rr_dataset(directory_path):
    """
    Loads the WN18RR dataset from train.txt, valid.txt, and test.txt files.
    """
    entity_set, relation_set = set(), set()

    # First, read all files to build a complete dictionary of all entities and relations
    for split in ["train", "valid", "test"]:
        with open(os.path.join(directory_path, f"{split}.txt"), "r") as f:
            for line in f:
    ...
    # Create mappings from the string names to integer IDs
    entity2idx = {name: i for i, name in enumerate(entity_set)}
    relation2idx = {name: i for i, name in enumerate(relation_set)}

    # Now, process each file again to create lists of integer triples
    def process_split(file_path):
        triples = []
        with open(file_path, "r") as f:
            for line in f:
                h, r, t = line.strip().split('\t')
                triples.append([entity2idx[h], relation2idx[r], entity2idx[t]])
        return np.array(triples)

    train_triples = process_split(os.path.join(directory_path, "train.txt"))
    valid_triples = process_split(os.path.join(directory_path, "valid.txt"))
    test_triples = process_split(os.path.join(directory_path, "test.txt"))
...
class OntologyAwareGATLayer(MessagePassing):
    def __init__(self, in_channels, out_channels, num_types, type_embedding_dim=32, heads=2, dropout=0.3):
...
class TransHModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types, heads=2):
        ...
        # Final embedding shape = GAT + GCN = 2 * embedding_dim
        self.refined_entity_dim = self.gat_out_dim * self.heads + self.gat_out_dim  # GAT + GCN

        self.entity_embeddings = nn.Embedding(num_entities, embedding_dim)
        self.relation_embeddings = nn.Embedding(num_relations, self.refined_entity_dim)
        self.hyperplane_embeddings = nn.Embedding(num_relations, self.refined_entity_dim)
        ...
        self.batch_norm = nn.BatchNorm1d(self.refined_entity_dim)
        ...
        layer_sizes = [self.refined_entity_dim * 4, 2048, 1024, 512, 256, 128]
    ...
    def forward(self, h, r, t):
        ...
        refined_embeddings = torch.cat([refined_gat, refined_gcn], dim=1)  # [N, 2*embedding_dim]
        ...
        # TransH projection
        proj_vec = F.normalize(self.hyperplane_embeddings(r), dim=-1)
        h_dot = (h_embed * proj_vec).sum(dim=1, keepdim=True)
    ...
    def score_triple(self, h, r, t):
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("edge_index and entity_type_tensor must be set before calling score_triple()")
        ...
        refined_entity_emb = torch.cat([refined_gat, refined_gcn], dim=1)

        h_embed = refined_entity_emb[h]
        r_embed = self.relation_embeddings(r)
        t_embed = refined_entity_emb[t]
        ...
        return self.fc_out(x).squeeze(-1)  # Raw plausibility score (no sigmoid)

# --- NEW: Load the WN18RR dataset ---

# 2. Load the dataset using the new function
print("Loading WN18RR dataset...")
wn18rr_data = load_wn18rr_dataset(WN18RR_DIR)
num_entities = wn18rr_data["num_entities"]
num_relations = wn18rr_data["num_relations"]
entity2idx = wn18rr_data["entity2idx"]
relation2idx = wn18rr_data["relation2idx"]
train_triples = wn18rr_data["train"] # This is a numpy array
valid_triples = wn18rr_data["valid"]
test_triples = wn18rr_data["test"]
# The 'triples' variable is used later to build the GNN graph structure
triples = np.concatenate([train_triples, valid_triples, test_triples], axis=0)
print(f"Loaded WN18RR with {num_entities} entities and {num_relations} relations.")

print("Creating placeholder variables for WN18RR...")
constraints = {}
entity_type_map = {}
idx2entity = {i: name for name, i in entity2idx.items()} # Build reverse map
idx2relation = {i: name for name, i in relation2idx.items()} # Build reverse map
type2idx, constraints_idx, entity_type_map_idx = {}, {}, {}
num_types = 1 # Use a dummy value of 1
...
# === Hybrid negative sampling for WN18RR ===
print("Generating hybrid negative samples for WN18RR using train set...")

# NOTE: We generate negatives ONLY from the training positives
train_pos_np = wn18rr_data["train"]

# Method 1: Flipped Triples
flipped_negatives = flip_entities(train_pos_np)

# Method 2: Popularity-based Noise
popularity_negatives = popularity_based_sampling(
    triples=train_pos_np.tolist(),  # .tolist() is safer for the Counter
    num_entities=num_entities,
    num_relations=num_relations,
    batch_size=len(train_pos_np)
)

# Method 3: Uniform Corruption (as the third component)
rng = np.random.default_rng(SEED)
uniform_negatives = np.copy(train_pos_np)
uniform_negatives[:, 2] = rng.integers(0, num_entities, size=len(train_pos_np))

# Merge the general-purpose negative samples
...
# This code correctly uses the pre-defined splits for WN18RR

# Get the positive triples for each split (as numpy arrays)
train_pos_np = wn18rr_data["train"]
valid_pos_np = wn18rr_data["valid"]
test_pos_np = wn18rr_data["test"]

# Use the 'negative_triples' array we created in the previous step
# It already contains a hybrid mix of flipped, popularity, and uniform negatives
# We will split these negatives to match the proportions of the positive splits

num_train = len(train_pos_np)
num_valid = len(valid_pos_np)

# Assign negatives to train, validation, and test sets
train_neg_np = negative_triples[:num_train]
valid_neg_np = negative_triples[num_train : num_train + num_valid]
test_neg_np = negative_triples[num_train + num_valid :]


# --- Create the final PyTorch tensors ---

# Training Set
X_train = torch.from_numpy(np.concatenate([train_pos_np, train_neg_np])).long().to(DEVICE)
y_train = torch.tensor([1]*len(train_pos_np) + [0]*len(train_neg_np), dtype=torch.float32).to(DEVICE)

# Validation Set
X_val = torch.from_numpy(np.concatenate([valid_pos_np, valid_neg_np])).long().to(DEVICE)
y_val = torch.tensor([1]*len(valid_pos_np) + [0]*len(valid_neg_np), dtype=torch.float32).to(DEVICE)

# Test Set
X_test = torch.from_numpy(np.concatenate([test_pos_np, test_neg_np])).long().to(DEVICE)
y_test = torch.tensor([1]*len(test_pos_np) + [0]*len(test_neg_np), dtype=torch.float32).to(DEVICE)

print("Datasets for WN18RR created using pre-defined splits.")
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    """
    Train a KG model using HybridLossWithSemantics (BCE + margin ranking + semantic penalty).
    Includes early stopping, learning rate scheduling, and loss plotting.
    """
    ...
    for epoch in trange(epochs, desc="Epochs"):
        ...
        tqdm_bar = tqdm(range(0, len(X_train), batch_size), desc=f"Epoch {epoch + 1}", leave=False)
        for i in tqdm_bar:
        ...
        if avg_val_loss < best_val_loss:
            ...
            tqdm.write(f" New best model saved at epoch {epoch+1} with validation loss {avg_val_loss:.4f}")
    ...
    plt.close()
    tqdm.write(f" Training & Validation loss plot saved to {os.path.join(plots_dir, 'loss_plot.png')}")

    # === Load best model ===
    model.load_state_dict(torch.load(os.path.join(FULL_MODEL_DIR, "best_model.pth")))
    tqdm.write(f" Best model from epoch {epoch + 1 - patience_counter} loaded for evaluation.")
    return model
...
classifier = TransHModel    (
    ...
    num_types=1  # from your `type2idx`
).to(DEVICE)

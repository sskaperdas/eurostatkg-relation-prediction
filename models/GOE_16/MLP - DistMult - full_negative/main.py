...
class DistMultModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types, dropout_rates=None):
        ...
        self.gat_proj = nn.Linear(64 * 4, embedding_dim)  # Project from GAT output back to emb_dim
    ...
    def forward(self, x):
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("GAT/GCN requires edge_index and entity_type_tensor to be set.")

        base_entity_emb = self.entity_embeddings.weight
        entity_types = self.entity_type_tensor.to(base_entity_emb.device)

        # === GAT
        gat_out = self.gat_layer(base_entity_emb, edge_index=self.edge_index, entity_types=entity_types)
        gat_out = self.gat_proj(gat_out)

        # === GCN
        gcn_out = self.gcn_layer(base_entity_emb, edge_index=self.edge_index, entity_types=entity_types)

        # === Mean fusion
        refined_entity_emb = (gat_out + gcn_out) / 2
        ...
        for fc, drop in zip(self.fc_layers, self.dropout_layers):
            score = F.relu(fc(score))
            score = drop(score)

        return torch.sigmoid(self.out(score))  # (batch_size, 1)

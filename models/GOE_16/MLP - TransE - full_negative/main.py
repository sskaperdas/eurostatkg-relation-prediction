...
BASE_DIR = "models"
PROJECT_DIR = "GOE_16"
MODEL_NAME = "MLP - TransE - full_negative"
...
class TransEModel(nn.Module):
    def __init__(self, num_entities, num_relations, num_types, embedding_dim,
                 dropout_rates=None, type_embedding_dim=32, heads=4):
        super(TransEModel, self).__init__()
        self.embedding_dim = embedding_dim
        self.type_embedding_dim = type_embedding_dim
        self.heads = heads
        self.refined_dim = embedding_dim * heads  # 512 if 256×2

        # === Embeddings ===
        self.entity_embeddings = nn.Embedding(num_entities, embedding_dim)
        self.relation_embeddings = nn.Embedding(num_relations, embedding_dim)
        self.relation_proj = nn.Linear(embedding_dim, self.refined_dim)  # project relation to match entities
        ...
        self.batch_norm = nn.BatchNorm1d(self.refined_dim)

        # === FC Stack ===
        if dropout_rates is None:
            dropout_rates = [0.5, 0.5, 0.5, 0.3, 0.2]

        input_dim = self.refined_dim * 3 + self.refined_dim  # h + t + predicted_t + r → 1792
        layer_sizes = [input_dim, 2048, 1024, 512, 256, 128]
        ...
        for i in range(len(layer_sizes) - 1):
            ...
            self.residual_projections.append(nn.Linear(layer_sizes[i], layer_sizes[i + 1]))
    ...
    def forward(self, h, r, t):
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("GCN inputs (edge_index, entity_type_tensor) must be set before forward.")
        ...
        refined_emb = (gat_out + gcn_out) / 2  # shape: [num_entities, 512]

        h_embed = refined_emb[h]
        t_embed = refined_emb[t]
        r_embed = self.relation_proj(self.relation_embeddings(r))  # [B, 512]
        ...
        x = torch.cat([h_embed, r_embed, t_embed, predicted_t], dim=1)  # [B, 1792]
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    """
    Train model with HybridLossWithSemantics (BCE + Ranking + Semantic), LR scheduler, and early stopping.
    """

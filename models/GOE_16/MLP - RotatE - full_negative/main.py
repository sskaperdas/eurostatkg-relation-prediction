...
BASE_DIR = "models"
PROJECT_DIR = "GOE_16"
MODEL_NAME = "MLP - RotatE - full_negative"
...
class RotatE_MLP_Model(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types, dropout_rates=None):
        ...
        # === Entity & Relation Embeddings (RotatE) ===
        ...
        self.relation_embeddings = nn.Embedding(num_relations, embedding_dim)  # angles in radians

        # === Ontology-aware GAT Layer ===
        ...
        self.gat_proj = nn.Linear(self.embedding_dim * self.gat_layer.heads, self.embedding_dim)
        self.gcn_proj = nn.Linear(self.embedding_dim, self.embedding_dim)  # GCN already outputs [N, 128]

        nn.init.xavier_uniform_(self.gat_proj.weight)
        nn.init.xavier_uniform_(self.gcn_proj.weight)
        ...
        for i in range(len(layer_sizes) - 1):
            ...
            dropout_rate = dropout_rates[i] if i < len(dropout_rates) else 0.2
            self.dropout_layers.append(nn.Dropout(dropout_rate))

        self.fc_out = nn.Linear(layer_sizes[-1], 1)

        # === External GAT/GCN Inputs ===
    ...
    def forward(self, h, r, t):
        ...
        # === Base Embeddings ===
        base_real = self.entity_embeddings_real.weight
        base_imag = self.entity_embeddings_imag.weight

        # === GAT Refinement ===
        gat_real_raw = self.gat_layer(base_real, self.edge_index, self.entity_type_tensor.to(base_real.device))
        gat_imag_raw = self.gat_layer(base_imag, self.edge_index, self.entity_type_tensor.to(base_imag.device))

        gat_real = self.gat_proj(gat_real_raw)  # Projected to [N, embedding_dim]
        gat_imag = self.gat_proj(gat_imag_raw)

        # === GCN Refinement ===
        gcn_real_raw = self.gcn_layer(base_real, self.edge_index, self.entity_type_tensor.to(base_real.device))
        gcn_imag_raw = self.gcn_layer(base_imag, self.edge_index, self.entity_type_tensor.to(base_imag.device))

        gcn_real = self.gcn_proj(gcn_real_raw)  # Projected to [N, embedding_dim]
        gcn_imag = self.gcn_proj(gcn_imag_raw)

        # === Combine (Average) ===
        refined_real = (gat_real + gcn_real) / 2
        refined_imag = (gat_imag + gcn_imag) / 2

        # === Get Embeddings ===
        h_real = refined_real[h]
        h_imag = refined_imag[h]
        t_real = refined_real[t]
        t_imag = refined_imag[t]
        theta = self.relation_embeddings(r) * self.pi

        # === RotatE Computation ===
        r_real = torch.cos(theta)
        r_imag = torch.sin(theta)

        h_mod = torch.sqrt(h_real ** 2 + h_imag ** 2 + 1e-9)
        t_mod = torch.sqrt(t_real ** 2 + t_imag ** 2 + 1e-9)
        ...
        rot_t_real = h_real * r_real - h_imag * r_imag
        rot_t_imag = h_real * r_imag + h_imag * r_real

        diff_real = rot_t_real - t_real
        diff_imag = rot_t_imag - t_imag
        score = torch.norm(diff_real, p=2, dim=-1) + torch.norm(diff_imag, p=2, dim=-1)

        # === MLP Processing ===
        concat_embed = torch.cat([rot_t_real, rot_t_imag, r_real, r_imag], dim=1)
        x = self.batch_norm(concat_embed)
    ...
    def score_triple(self, h, r, t):
        """
        Computes raw plausibility score (pre-sigmoid) for (h, r, t)
        using RotatE logic + same MLP path as forward().
        """
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("GCN edge_index and entity_type_tensor must be set before calling score_triple().")

        # === Apply GAT to refine embeddings ===
        base_real = self.entity_embeddings_real.weight
        base_imag = self.entity_embeddings_imag.weight

        refined_real = self.gat_layer(
            base_real,
            edge_index=self.edge_index,
            entity_types=self.entity_type_tensor.to(base_real.device)
        )

        refined_imag = self.gat_layer(
            base_imag,
            edge_index=self.edge_index,
            entity_types=self.entity_type_tensor.to(base_imag.device)
        )

        # === Get embeddings for h, r, t ===
        h_real = refined_real[h]
        h_imag = refined_imag[h]
        t_real = refined_real[t]
        t_imag = refined_imag[t]
        ...
        return self.fc_out(x).squeeze(-1)  # Pre-sigmoid plausibility score
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    Includes early stopping and loss plotting.
    """

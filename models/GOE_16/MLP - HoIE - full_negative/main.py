...
class MLPHoIE(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types, dropout_rates=None):
        ...
        self.gat_proj = nn.Linear(embedding_dim * 2, embedding_dim)
        ...
        # === Batch Norm on HoIE input ===
        self.batch_norm = nn.BatchNorm1d(embedding_dim * 3)  # 256 * 3 = 768
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    """
    Train model using HybridLossWithSemantics (BCE + MarginRanking + Semantic penalty),
    with early stopping, tqdm progress bars, and loss visualization.
    """
    ...
    for epoch in trange(epochs, desc="Epochs", dynamic_ncols=True):  # ✅ tqdm for outer loop
        model.train()
        epoch_loss = 0.0
        num_batches = 0
        ...
        batch_iter = tqdm(range(0, len(X_train), batch_size), desc=f"Train {epoch+1}",
                          leave=False, position=1, dynamic_ncols=True)  # ✅ tqdm for inner loop
    ...
    # === Plotting ===
    os.makedirs(os.path.join(FULL_MODEL_DIR, "plots"), exist_ok=True)
    ...
    plt.grid(alpha=0.3)
    plt.savefig(os.path.join(FULL_MODEL_DIR, "plots", "loss_plot.png"), dpi=300, bbox_inches="tight")
    plt.close()

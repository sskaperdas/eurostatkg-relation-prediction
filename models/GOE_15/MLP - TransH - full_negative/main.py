...
class TransHModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types, heads=2):
        ...
        self.final_entity_dim = self.gat_out_dim * self.heads

        self.entity_embeddings = nn.Embedding(num_entities, embedding_dim)
        self.relation_embeddings = nn.Embedding(num_relations, self.final_entity_dim)
        self.hyperplane_embeddings = nn.Embedding(num_relations, self.final_entity_dim * self.final_entity_dim)
    ...
    def forward(self, h, r, t):
        ...
        h_proj = torch.bmm(hyperplane_embed, h_embed.unsqueeze(-1)).squeeze(-1)
        t_proj = torch.bmm(hyperplane_embed, t_embed.unsqueeze(-1)).squeeze(-1)
    ...
    def score_triple(self, h, r, t):
        ...
        using the same logic as forward(), including TransH projection.
        """
        if self.edge_index is None or self.entity_type_tensor is None:
        ...
        # Apply TransH projection using hyperplane
        hyperplane_embed = self.hyperplane_embeddings(r).view(-1, h_embed.shape[1], h_embed.shape[1])
        h_proj = torch.bmm(hyperplane_embed, h_embed.unsqueeze(-1)).squeeze(-1)
        t_proj = torch.bmm(hyperplane_embed, t_embed.unsqueeze(-1)).squeeze(-1)
        ...
        return self.fc_out(x).squeeze(-1)  # Raw plausibility score

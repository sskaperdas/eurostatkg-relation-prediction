...
class TransEModel(nn.Module):
    def __init__(self, num_entities, num_relations, num_types, embedding_dim, dropout_rates=None, type_embedding_dim=32, heads=2):
        ...
        self.relation_proj = nn.Linear(embedding_dim, embedding_dim * heads)
        ...
        # Will be set externally before training
        ...
        self.batch_norm = nn.BatchNorm1d(embedding_dim * heads)

        # === FC Layer Architecture ===
        if dropout_rates is None:
            dropout_rates = [0.5, 0.5, 0.5, 0.3, 0.2]

        layer_sizes = [embedding_dim * heads * 4, 2048, 1024, 512, 256, 128]
    ...
    def forward(self, h, r, t):
        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("GCN inputs (edge_index, entity_type_tensor) must be set before calling forward")

        # === Apply Ontology-aware GAT ===
    ...
    def score_triple(self, h, r, t):
        ...
        using the same logic as forward().
        """
        if self.edge_index is None or self.entity_type_tensor is None:

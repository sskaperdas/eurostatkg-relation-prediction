...
BASE_DIR = "models"
PROJECT_DIR = "GOE_9"
MODEL_NAME = "Conv1D - RotatE - Negative 1"
...
class ComplexModelWithRotatE(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, rot_embedding_dim, num_types, dropout_rate=0.5, l2_reg=0.01):
        ...
        self.gat_layer = OntologyAwareGATLayer(
            ...
            heads=1,  # can increase to 2+ if desired
            dropout=0.3
        )
...
trained_classifier = ComplexModelWithRotatE    (
...
# === Inject into model ===
trained_classifier.edge_index = edge_index
trained_classifier.entity_type_tensor = entity_type_tensor

# Force lazy layer initialization
trained_classifier.eval()
_ = trained_classifier(torch.tensor([[0, 0, 0]], device=DEVICE))
...
hits_results = calculate_hits_metrics(
    trained_classifier,
    X_test,
    y_test,
    epsilons=[0.01, 0.05, 0.1]
...
with open(os.path.join(FULL_MODEL_DIR, "logs", "partial_hits_results.json"), "w") as f:
    json.dump(hits_results, f, indent=4)
...
mrr_results = calculate_mrr_metrics(
    trained_classifier,
    X_test,
    y_test,
    epsilons=[0.01, 0.05, 0.1]
...
with open(os.path.join(FULL_MODEL_DIR, "logs", "partial_mrr_results.json"), "w") as f:
    json.dump(hits_results, f, indent=4)
...
mean_rank_results = calculate_mean_rank_metrics(
    trained_classifier,
    X_test,
    y_test,
    epsilons=[0.01, 0.05, 0.1]
...
with open(os.path.join(FULL_MODEL_DIR, "logs", "partial_mean_rank_results.json"), "w") as f:
    json.dump(hits_results, f, indent=4)
...
ndcg_results = calculate_ndcg_metrics(
    trained_classifier,
    X_test,
    y_test,
    epsilons=[0.01, 0.05, 0.1],
...
with open(os.path.join(FULL_MODEL_DIR, "logs", "partial_ndcg_results.json"), "w") as f:
...
median_rank_results = calculate_median_rank_metrics(
    trained_classifier,
    X_test,
    y_test,
    epsilons=[0.01, 0.05, 0.1]
...
with open(os.path.join(FULL_MODEL_DIR, "logs", "partial_median_rank_results.json"), "w") as f:
    json.dump(median_rank_results, f, indent=4)

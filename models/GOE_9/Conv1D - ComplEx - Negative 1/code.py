...
BASE_DIR = "models"
PROJECT_DIR = "GOE_9"
MODEL_NAME = "Conv1D - ComplEx - Negative 1"
...
class ComplExModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types, dropout_rate=0.5):
        ...
        # === Dummy Tensor to determine flatten size ===
        self.gat_heads = 4  # or make it a parameter
        concat_dim = embedding_dim * self.gat_heads * 4
        dummy_input = torch.zeros(1, 1, concat_dim)
        dummy_output = self.conv1d_2(
            self.dropout1(F.relu(self.batch_norm1(self.conv1d_1(dummy_input))))
        )
    ...
    def forward(self, inputs):
        s_idx, p_idx, _ = inputs[:, 0], inputs[:, 1], inputs[:, 2]

        if self.edge_index is None or self.entity_type_tensor is None:
            raise ValueError("GAT requires edge_index and entity_type_tensor to be set before forward().")
        ...
        # === Use refined embeddings ===
        s_real = refined_real[s_idx]
        s_imag = refined_imag[s_idx]
        p_real = self.relation_real_embeddings(p_idx)
        p_imag = self.relation_imag_embeddings(p_idx)

        # Concatenation
        s_embed = torch.cat([s_real, s_imag], dim=1)
        p_embed = torch.cat([p_real, p_imag], dim=1)

        concat = torch.cat([s_embed, p_embed], dim=1)  # shape: (batch, 4 * emb_dim)

        # === Conv1D + MLP ===
        x = concat.unsqueeze(1)  # shape: (batch, 1, 4 * emb_dim)
...
def calculate_hits_metrics(model, X_test, y_test, epsilons=None, batch_size=512):
    ...
    model.eval()
    num_relations = model.relation_imag_embeddings.num_embeddings

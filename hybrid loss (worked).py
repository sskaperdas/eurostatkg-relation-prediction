...
BASE_DIR = "models"
PROJECT_DIR = "GOE_1"
MODEL_NAME = "hybrid loss (worked) - gcn"
...
class TransEConvModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim, num_types=0, class_embedding_dim=32,
                 gcn_out_dim=128, dropout_rate=0.5, edge_index=None, entity_type_tensor=None):
        ...
        self.class_embeddings = nn.Embedding(num_types, class_embedding_dim)
        self.gate_linear = nn.Linear(embedding_dim, class_embedding_dim)
        self.entity_id_to_class_ids = None  # Set externally

        # === GCN injection ===
        self.use_gcn = True
        self.edge_index = edge_index
        self.entity_type_tensor = entity_type_tensor
        self.gcn = OntologyAwareGCNLayer(
        ...
        nn.init.xavier_uniform_(self.class_embeddings.weight)
    ...
    def apply_gcn(self):
        if self.use_gcn:
            with torch.no_grad():
                base_embeds = self.entity_embeddings.weight  # [num_entities, D]
                refined = self.gcn(base_embeds, self.edge_index, self.entity_type_tensor)
                self.entity_embeddings.weight.data = refined  # overwrite
            self.use_gcn = False  # only apply once unless retraining
    ...
    def forward(self, inputs):
        ...
        s_embed = self.entity_embeddings(s)
        p_embed = self.relation_embeddings(p)
        o_embed = self.entity_embeddings(o)

        if self.training or self.entity_id_to_class_ids:
            s_class = get_class_embedding_batch(s, self.entity_id_to_class_ids, self.class_embeddings, inputs.device)
            o_class = get_class_embedding_batch(o, self.entity_id_to_class_ids, self.class_embeddings, inputs.device)
            s_gate = torch.sigmoid(self.gate_linear(s_embed))
            o_gate = torch.sigmoid(self.gate_linear(o_embed))
            s_embed = torch.cat([s_embed, s_gate * s_class], dim=1)
            o_embed = torch.cat([o_embed, o_gate * o_class], dim=1)
            p_embed = F.pad(p_embed, (0, self.class_embedding_dim))
        else:
            s_pad = torch.zeros(s.shape[0], self.class_embedding_dim, device=inputs.device)
            o_pad = torch.zeros(o.shape[0], self.class_embedding_dim, device=inputs.device)
            s_embed = torch.cat([s_embed, s_pad], dim=1)
            o_embed = torch.cat([o_embed, o_pad], dim=1)
            p_embed = F.pad(p_embed, (0, self.class_embedding_dim))

        predicted_o = s_embed + p_embed
        x = torch.cat([s_embed, p_embed, o_embed, predicted_o], dim=1).unsqueeze(1)
    ...
    def score_triple(self, h, r, t):
        ...
        o_embed = self.entity_embeddings(t)

        if self.training or self.entity_id_to_class_ids:
            s_class = get_class_embedding_batch(h, self.entity_id_to_class_ids, self.class_embeddings, h.device)
            o_class = get_class_embedding_batch(t, self.entity_id_to_class_ids, self.class_embeddings, t.device)
            ...
            s_pad = torch.zeros(h.shape[0], self.class_embedding_dim, device=h.device)
            o_pad = torch.zeros(t.shape[0], self.class_embedding_dim, device=t.device)
            s_embed = torch.cat([s_embed, s_pad], dim=1)
            o_embed = torch.cat([o_embed, o_pad], dim=1)
            p_embed = F.pad(p_embed, (0, self.class_embedding_dim))

        return -torch.norm(s_embed + p_embed - o_embed, p=2, dim=1)
...
def fast_generate_ontology_aware_negatives(
    triples,                  # np.array [N, 3]
    constraints_idx,          # dict[int → int]: relation_idx → expected_range_type_idx
    entity_type_map_idx,      # dict[int → set[int]]: entity_idx → set of type_idx
    entity_popularity=None,   # np.array[num_entities]
    num_entities=0,
    max_samples=None,
    popularity_bias=0.8,
    rng=None
    ...
    """
    Fast negative triple generation with popularity and ontology-aware type violations.

    Returns:
        np.array [N, 3] negative triples
    """
    ...
    if max_samples is None:
        max_samples = num_triples

    # === Build sampling distribution ===
    if entity_popularity is not None:
        popularity = entity_popularity / (entity_popularity.sum() + 1e-8)
        uniform = np.ones(num_entities) / num_entities
        sampling_probs = popularity_bias * popularity + (1 - popularity_bias) * uniform
    else:
        sampling_probs = np.ones(num_entities) / num_entities

    # === Sample ===
    sample_indices = rng.choice(num_triples, size=max_samples, replace=False)
    corrupted_triples = []

    for idx in sample_indices:
        s, p, o = triples[idx]
        expected_range = constraints_idx.get(p, None)

        for _ in range(5):
            corrupt_o = int(rng.choice(num_entities, p=sampling_probs))
            if corrupt_o == o:
                continue

            if expected_range is not None:
                types = entity_type_map_idx.get(corrupt_o, set())
                if expected_range not in types:
                    corrupted_triples.append((s, p, corrupt_o))
                    break
            else:
                corrupted_triples.append((s, p, corrupt_o))
                break

    return np.array(corrupted_triples)
...
def get_class_embedding_batch(entity_ids, entity_id_to_class_ids, class_embeddings, device, max_classes=5):
    """
    entity_ids: Tensor of shape [B]
    entity_id_to_class_ids: dict[int → list[int]]
    class_embeddings: nn.Embedding layer
    Returns: Tensor of shape [B, class_embedding_dim]
    """
    batch_embeddings = []

    for eid in entity_ids.tolist():
        class_ids = entity_id_to_class_ids.get(eid, [])
        if len(class_ids) == 0:
            emb = torch.zeros(class_embeddings.embedding_dim, device=device)
        else:
            class_tensor = torch.tensor(class_ids[:max_classes], device=device)
            emb = class_embeddings(class_tensor).mean(dim=0)
        batch_embeddings.append(emb)

    return torch.stack(batch_embeddings, dim=0)  # [B, D_class]
...
negative_triples = fast_generate_ontology_aware_negatives(
    triples=np.array(triples),
    constraints_idx=constraints_idx,
    entity_type_map_idx=entity_type_map_idx,
    entity_popularity=entity_counts,
...
X_temp, X_test, y_temp, y_test = train_test_split(
    all_triples, labels, test_size=0.15, random_state=42, stratify=labels
)

# Step 2: From remaining 85%, split 15% val = 0.15 / 0.85 ≈ 0.1765
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    for epoch in range(epochs):
        ...
        progress_bar = tqdm(range(0, len(X_train), batch_size), desc=f"Epoch {epoch + 1}/{epochs}", leave=False)
        ...
        for i in progress_bar:
            ...
            batch_input = torch.stack([h, r, t], dim=1)  # shape: [batch_size, 3]
            pred_probs = model(batch_input).view(-1)
            pos_mask = (batch_y == 1)
            h_pos, r_pos, t_pos = h[pos_mask], r[pos_mask], t[pos_mask]

            if len(h_pos) > 0:
                t_neg = torch.randint(0, model.entity_embeddings.num_embeddings, t_pos.shape, device=DEVICE)
                pos_triples = torch.stack([h_pos, r_pos, t_pos], dim=1)
                neg_triples = torch.stack([h_pos, r_pos, t_neg], dim=1)

                total_loss = loss_fn(
                    pred_probs, batch_y, model,
                    pos_triples, neg_triples,
                    idx2entity, idx2relation,
                    constraints, entity_type_map
                )
            else:
                # Fallback to just BCE loss when no positives in batch
                total_loss = F.binary_cross_entropy(pred_probs, batch_y.float())
        ...
        with torch.no_grad():
            for j in range(0, len(X_val), batch_size):
                ...
                val_input = torch.stack([h_val, r_val, t_val], dim=1)
                val_preds = model(val_input).view(-1)
                loss_val = bce_loss_only(val_preds, batch_y_val.float())
...
# Build indexed multi-type map
entity_id_to_class_ids = {}

for entity_uri, types in entity_type_map.items():
    if entity_uri not in entity2idx:
        continue
    entity_id = entity2idx[entity_uri]
    class_ids = [type2idx[t] for t in types if t in type2idx]
    if class_ids:
        entity_id_to_class_ids[entity_id] = class_ids

# Train Classification Model
# Build a sparse edge_index (PyG format)
def build_edge_index(triples, num_entities):
    edge_list = []
    for h, _, t in triples:
        edge_list.append((h, t))
        edge_list.append((t, h))  # undirected
    edge_index = torch.tensor(edge_list, dtype=torch.long).T
    return edge_index

# Prepare entity_type_tensor: [num_entities]
entity_type_tensor = torch.zeros(num_entities, dtype=torch.long)
for eid, types in entity_type_map_idx.items():
    entity_type_tensor[eid] = list(types)[0]  # pick one type

edge_index = build_edge_index(triples, num_entities).to(DEVICE)
entity_type_tensor = entity_type_tensor.to(DEVICE)

classifier = TransEConvModel(
    num_entities=num_entities,
    ...
    entity_type_tensor=entity_type_tensor
).to(DEVICE)
classifier.entity_id_to_class_ids = entity_id_to_class_ids
...
def evaluate_model(model, X_test, y_test, model_name="TransEModel", batch_size=512):
    ...
    with torch.no_grad():
        for i in range(0, len(X_test), batch_size):
            ...
            h, r, t = batch_X[:, 0], batch_X[:, 1], batch_X[:, 2]
            batch_input = torch.stack([h, r, t], dim=1)
    ...
    with torch.no_grad():
        for i in range(0, len(X_test), batch_size):
            ...
            for j in range(len(batch_X)):
                ...
                triple_batch = torch.stack([h_expand, all_r, t_expand], dim=1)  # shape: [num_relations, 3]
                all_scores = model(triple_batch).squeeze()
    ...
    with torch.no_grad():
        for i in range(0, len(X_test), batch_size):
            ...
            for j in range(len(batch_X)):
                ...
                triple_batch = torch.stack([h_expand, all_r, t_expand], dim=1)  # shape: [num_relations, 3]
                scores = model(triple_batch).squeeze()  # shape: [num_relations]
